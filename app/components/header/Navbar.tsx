      ...
      <ul className='flex'>
        {filteredLinks.map((link, index) => (
          <li key={index}>
            <Link
              type='nav'
              className='flex justify-center items-center text-xl gold-heading font-bold textShadowMd rounded-full w-36 h-10 transition duration-200 ease-in-out hover:opacity-70'
              href={link.href}>
              {link.title}
            </Link>
          </li>
